...
#define key {123, 15, 200}
#define keySize 3

//This Function will calculate MAC of a given block and will return MAC of that block
...
void Perform_block_wise_mac(fstream& file, long long blockSize, const vector<GF256::Element>& alpha, vector<int> &block) {
    ...
    ofstream macFile("mac.bin", ios::binary);
    if (!macFile.is_open()) {
...
void Pick_block_wise_mac(fstream& file, long long blockSize, vector<int> &block) {
    long long m2 = blockSize; // Number of sectors in MAC
...
bool compareFiles(const std::string& file1, const std::string& file2) {
    ...
    constexpr size_t bufferSize = 1024; // Adjust the buffer size as needed
...
void proofGen(vector<GF256::Element> &v, fstream &tag, vector<int> &block) 
{
    if (!tag.is_open()) {
    ...
    long long fileSize = tag.tellg();
    tag.seekg(0, std::ios::beg);

    long long noc = ceil((double)fileSize/ v.size());
    long long m1 = v.size();

    vector<GF256::Element> result(noc, GF256::Element(0));

    Perform_block_wise_v_mac(tag, noc, result, v, keySize);
    ...
    if (resultFile.is_open()) {
        ...
        resultFile.close();
        cout << "\nResult vector stored in result.bin" << endl;
    } else {
...
{
    Perform_block_wise_mac(file, sector, alpha, block);
}

void tagPick(fstream &file, vector<GF256::Element> &alpha, vector<int> &block)
{
    int blockSize= alpha.size();
    Pick_block_wise_mac(file, blockSize, block);
}
...
bool verifyProof(vector<GF256::Element> &v, fstream &mu, fstream &tag, vector<GF256::Element> &alpha, vector<int> &block) 
{
    ...
    if (std::rename("Mul_VF.bin", "Mul_V_Macf.bin") != 0) {             //As proofGen algorithm has now calculated V x MAC(f)
